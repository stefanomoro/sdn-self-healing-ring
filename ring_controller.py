...
class switch(app_manager.RyuApp):
    ...
    def get_topology_data(self, ev):
		...
		if contatore_TD == len(switches): #quando ha finito il topology discovery
			...
			for i in range(len(switches)): #for da i cicli per i switch [i=0->5]
				sw_id_locked = sw_id
				for link in links: #for per scorrere tutti i link trovati dal TD
					if (link[0] == sw_id_locked and (link[1] == routing_matrix[i-1][0] or i == 0)):
						routing_matrix[i][2] = link[2]
					if (link[0] == sw_id and sw_id == sw_id_locked and (link[1] != routing_matrix[i-1][0] or i == 0)):
